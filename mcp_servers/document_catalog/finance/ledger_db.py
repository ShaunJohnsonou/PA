...
class FinanceLedger:
    ...
    def insert_transactions(self, transactions: list[dict]) -> int:
        """Bulk-insert transaction rows. Returns count inserted."""
        if not transactions:
            logger.debug("insert_transactions: no transactions to insert")
            return 0

        logger.debug("insert_transactions: inserting %d rows...", len(transactions))
        ...
        logger.info("Inserted %d transactions", len(rows))
        return len(rows)
